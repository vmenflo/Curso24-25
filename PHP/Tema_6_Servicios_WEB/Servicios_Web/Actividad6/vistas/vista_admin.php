...
if(isset($_POST["btnNuevo"])||isset($_POST["btnContNuevo"])||isset($_POST["btnEditar"])||isset($_POST["btnContEditar"]))
{
    ...
    if(isset($json_familias["no_auth"]))
    {
        session_unset();
        $_SESSION["mensaje_seguridad"]="El tiempo de sesión de la API ha caducado";
        header("Location:index.php");
    ...
    {
        session_destroy();
        die(error_page("Actividad 6","<p>".$json_familias["error"]."</p>"));
    }

    if(isset($json_familias["mensaje_baneo"]))
    {
        session_unset();//Me deslogueo
        $_SESSION["mensaje_seguridad"]="Usted ya no se encuentra registrado en la BD";
        header("Location:index.php");
...
{
    session_destroy();
    die(error_page("Actividad 6","<p>".$json_productos["error"]."</p>"));
}
